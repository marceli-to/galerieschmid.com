    ...
    if ($table->data)
    {
      foreach($table->data as $item)
      {
        $exhibition = $this->model::create([
          ...
          'date_start' => $item->DATUM_VON && $item->DATUM_VON > 0 && $item->DATUM_VON <= now() ? date('Y-m-d', $item->DATUM_VON + 86400) : null,
          'date_end' => $item->DATUM_BIS && $item->DATUM_BIS > 0 && $item->DATUM_BIS <= now() ? date('Y-m-d', $item->DATUM_BIS + 86400) : null,
